...
volatile Product products[] = {
    {"i.Café Solo",      1.00, 0, false},
    {"ii.Café Cortado",   1.10, 1, false},
    {"iii.Café Doble",     1.25, 2, false},
    {"iv.Café Premium",   1.50, 3, false},
...
const Option admin_options[] = {
    {"i.Ver temperatura", 0, false},
...
const size_t ADMIN_OPTIONS_SIZE = sizeof(admin_options) / sizeof(admin_options[0]);
...
void joystick_read() {
  ...
  if (x > 200 && x < 900 && y > 920 && y < 1023) {
    if (current_admin_index < ADMIN_OPTIONS_SIZE - 1) {
      current_admin_index++;
      moved = true;
    }
  }
