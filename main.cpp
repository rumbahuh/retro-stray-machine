...
volatile Product products[] = {{"i.Cafe Solo", 1.00, 0, false},
...
void joystick_read(int choice) {
  ...
  if (moved_product) {
    ...
    display(current_product->line);
    lcd.print(" ");
    lcd.print(current_product->price);
  }
}
...
void cafeSolo(int time) {prepararBebida(time);}

void cafeCortado(int time) {prepararBebida(time); }

void cafeDoble(int time) {prepararBebida(time);}

void cafePremium(int time) {prepararBebida(time);}

void chocolate(int time) {prepararBebida(time);}
