...
bool previously_detected_person = false;
...
volatile size_t PRODUCTS_SIZE = sizeof(products) / sizeof(products[INITIAL_INDEX]);
...
void display(const char* message) {
  ...
  if (len > MAX_CHARPTR_PER_SCREEN) {
    lcd.setCursor(0, 1);
    for (int i = MAX_CHARPTR_PER_SCREEN; i < LCD_MAX_CHARS && message[i] != '\0'; i++) {
      lcd.print(message[i]);
    }
  }
}
